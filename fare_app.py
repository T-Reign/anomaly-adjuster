...
# --- 1. SEQUENCES ---
SEQUENCES = {
    "Reading-Aldershot": ["READING", "WOKINGHAM", "BRACKNELL", "ASCOT", "BAGSHOT", "CAMBERLEY", "FRIMLEY", "ASH VALE", "ALDERSHOT"],
    "Ascot-Guildford": ["ASCOT", "BAGSHOT", "CAMBERLEY", "FRIMLEY", "ASH VALE", "ASH", "WANBOROUGH", "GUILDFORD"],
    "London-Inbound": ["LONDON BR", "CLAPHAM JUNCTION LONDON", "VIRGINIA WATER", "ASCOT", "BAGSHOT"],
    "Aldershot-Oxshott-via-Surbiton": ["ALDERSHOT", "WOKING", "BYFLEET & NEW HAW", "WEYBRIDGE", "WALTON-ON-THAMES", "HERSHAM", "ESHER", "SURBITON", "HINCHLEY WOOD", "CLAYGATE", "OXSHOTT"]
}
                ...
                for j, near_st in enumerate(path[i+1:], i+1):
                    for k, far_st in enumerate(path[j+1:], j+1):
                        ...
                        if id_s in price_lookup and id_l in price_lookup:
                            ...
                            if p_s - p_l > 0.01:
                                lb_gaps.append({"Journey": f"{start_st} to...", "Near": near_st, "Far": far_st, "Gap": round(p_s - p_l, 2)})
        if lb_gaps:
            st.dataframe(pd.DataFrame(lb_gaps).sort_values('Gap', ascending=False).head(10), 
                         column_config={"Gap": st.column_config.NumberColumn("Gap", format="£%.2f")},
                         use_container_width=True, hide_index=True)
        else:
